...
describe Validators::DateInput do
  ...
  let(:window)  { 1.day + Validators::DateInput::WINDOW }
  let(:max_len) { Validators::DateInput::MAX_LEN }

  it 'should raise an error if the input is above the maximum size' do
    buffer = '1' * (max_len + 1)

    expect { parse_date(buffer) }
      .to raise_error(Validators::DateInput::DateTimeInputError)
  end
  ...
  it 'should raise an error if date is outside the expected window' do
    ...
    expect { parse_datetime(date_a.to_s) }
      .to raise_error(Validators::DateInput::DateTimeInputError)
    expect { parse_datetime(date_b.to_s) }
      .to raise_error(Validators::DateInput::DateTimeInputError)
  end
