#!/bin/sh
# Build.sh for older AWS Command line tool, as with Track a Query and People finder. Line 50 is different.
...
function _build() {
  ...
  region='eu-west-2'
  context='live-1'
  aws_profile='ecr-live-1-parliamentary-questions'
  ...
  docker_build_tag=pq-${current_branch}-${short_version}
  ...
  p "------------------------------------------------------------------------"
  p "Building PQ Tracker image for deployment"
